...
func seedSyncedCatalog(t *testing.T, nemHomeDir, catalogName string, entries []ocixtest.FakeEntry) {
	t.Helper()
	src, err := oci.New(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	ocixtest.PushFakeCatalog(t, src, entries, ocix.SchemaVersion)
