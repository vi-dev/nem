...
func TestComposeEnvNeverSetsLoaderPath(t *testing.T) {
	...
	ectx := EnvContext{
		Version: "v1", Platform: spec.Platform{OS: runtime.GOOS, Arch: runtime.GOARCH},
		Prefix: "/prefix", AbsRpath: true,
	}

	noAmbient := envMap(ComposeEnv(nil, []ResolvedDep{self}, ectx))
	for _, k := range []string{"LD_LIBRARY_PATH", "DYLD_LIBRARY_PATH"} {
		if v, ok := noAmbient[k]; ok {
			t.Fatalf("ComposeEnv must never set %s, got %q", k, v)
		}
	}

	base := []string{"LD_LIBRARY_PATH=sentinel", "DYLD_LIBRARY_PATH=sentinel"}
	withAmbient := envMap(ComposeEnv(base, []ResolvedDep{self}, ectx))
	for _, k := range []string{"LD_LIBRARY_PATH", "DYLD_LIBRARY_PATH"} {
