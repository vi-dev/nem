...
func releaseServer(t *testing.T, version, script string, badSum bool) (*Updater, *atomic.Int32) {
	...
	t.Cleanup(srv.Close)
	u := &Updater{Client: srv.Client(), DownloadBase: srv.URL + "/download", Repo: "vi-dev/nem"}
	return u, &requests
}
...
func TestResolveLatest(t *testing.T) {
	...
	u := &Updater{Client: srv.Client(), APIBase: srv.URL, Repo: "vi-dev/nem", Token: "tok123"}
	...
	if want := "/repos/vi-dev/nem/releases/latest"; gotPath != want {
...
func TestResolveLatestAPIError(t *testing.T) {
	...
	u := &Updater{Client: srv.Client(), APIBase: srv.URL, Repo: "vi-dev/nem"}
